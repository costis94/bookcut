...
def search_downloader(file, href):
    ...
    # progress bar
    buffer_size = 1024
    progress = tqdm(response.iter_content(buffer_size), f"{file}",total=total_size, unit="B", unit_scale=True, unit_divisor=1024)
...
def search(term):
    ...
    choices.append('c')
    while True:
        tell_me = str(input('\n \nPlease enter a number from 1 to {number} to download a book or press "C" to abort search: '.format(number = len(extensions))))
